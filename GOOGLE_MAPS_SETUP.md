...
2. Edit `.env` and add your API key:
   ...
   > Google **Advanced Markers**, which only position correctly on a map that has
   > a valid, Google-issued **Map ID**. Map IDs are generated by Google (you
   > cannot invent an arbitrary string) — create one under
   > [Maps Studio → Map IDs](https://console.cloud.google.com/google/maps-apis/studio/maps)
   > (Map type: *JavaScript*). With an invalid or missing Map ID the pins render
   > in the wrong location and appear to sit at a different zoom than the base
   > map. If `VITE_GOOGLE_MAPS_MAP_ID` is unset the app falls back to Google's
   > built-in `DEMO_MAP_ID`, which works for development but is not intended for
   > production.
